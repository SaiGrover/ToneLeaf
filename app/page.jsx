...
function Welcome({ onStart }) {
  return <section className="welcome" aria-labelledby="welcomeTitle">
    <div className="welcome-copy"><Brand /><p className="eyebrow">Private emotion intelligence</p><h1 id="welcomeTitle">Read between<br /><em>the lines.</em></h1><p className="welcome-lede">Toneleaf turns everyday language into gentle, useful emotional signals — without sending your words anywhere.</p><div className="welcome-actions"><button className="primary-button" onClick={onStart}>Explore your tone <span>→</span></button><span className="privacy-note"><span className="privacy-dot" /> Private by default</span></div></div>
...
function ResultPanel({ result, onCopy }) {
  ...
  const info = polarity ? getPolarityPresentation(result) : result.label === "distress"
    ...
    : { label: "Supportive / stable", glyph: "☺", description: "No strong distress pattern was detected in this text." };
  ...
  return <section className="results-panel panel" aria-live="polite"><div className="panel-heading"><div><span className="step">02</span><h3>Your result</h3></div><button className="copy-button" onClick={onCopy}>Copy summary</button></div><div className="results"><div className={`result-hero ${result.label} ${info.mixed ? "mixed" : ""}`}><div className="result-face">{info.glyph}</div><div><p>PRIMARY SIGNAL</p><h3>{info.label}</h3><span>{info.description}</span></div><div className="result-strength"><strong>{result.confidence}%</strong><small>signal share</small></div></div><div className="score-list">{scoreEntries.map(([key, value]) => <div className={`score-row ${key === "supportive" ? "positive" : key === "distress" ? "negative" : key}`} key={key}><div><span>{key === "supportive" ? "Supportive / stable" : title(key)}</span><b>{value}%</b></div><div className="track"><i style={{ width: `${value}%` }} /></div></div>)}</div><div className="insight"><span>✦</span><div><strong>What shaped this result</strong><p>{insight}</p></div></div>{!polarity && <p className="safety-note">This tool cannot diagnose a mental-health condition. If you or someone else may be in immediate danger, contact local emergency services or a trusted crisis service.</p>}</div></section>;
}
...
export default function Home() {
  ...
  const copyResult = async () => {
    ...
    const resultLabel = result.mode === "polarity" ? getPolarityPresentation(result).label : result.label === "distress" ? "Distress signal" : "Supportive / stable";
